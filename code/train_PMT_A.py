...
from dataloaders.la_heart import LAHeart, RandomCrop, ToTensor_o, ToTensor, TwoStreamBatchSampler, RandomRotFlip, Normalise, RandomBrightness, RandomBrightnessContrast, RandomGaussianNoise, WeakStrongAugment, RandomRotFlip_consistency
...
parser.add_argument("--exp", type=str, default="PMT_A", help="model_name") 
parser.add_argument(
...
parser.add_argument(
    "--base_lr", type=float, default=0.065, help="maximum epoch number to train"
) 
parser.add_argument(
...
max_score = 0
teacher_max_score = 0
randomRotFlip = RandomRotFlip_consistency()
...
def test_calculate_metric(model_array):
    with open(args.root_path + "/" + "test.list", "r") as f:  # todo change test flod
...
def rand_bbox(size, lam=None):
    ...
    bbd1 = np.clip(cz - cut_d // 2, 0, D)
    bbd2 = np.clip(cz + cut_d // 2, 0, D)
    return bbx1, bby1, bbx2, bby2, bbd1, bbd2

def cut_mix(volume=None, mask=None, gt=None): # x
    mix_volume = volume.clone()
    mix_target = mask.clone()
    if gt != None:
        mix_gt = gt.clone()
    u_rand_index = torch.randperm(volume.size()[0])[:volume.size()[0]].cuda()
    u_bbx1, u_bby1, u_bbx2, u_bby2, u_bbd1, u_bbd2 = rand_bbox(volume.size(), lam=np.random.beta(4, 4))
    for i in range(0, args.batch_size):
        if i < mix_volume.shape[0]:
            mix_volume[i, :, u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i], u_bbd1[i]:u_bbd2[i]] = \
                volume[u_rand_index[i], :, u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i], u_bbd1[i]:u_bbd2[i]]
            mix_target[i, :, u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i], u_bbd1[i]:u_bbd2[i]] = \
                mask[u_rand_index[i], :, u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i], u_bbd1[i]:u_bbd2[i]]
            if gt != None:
                mix_gt[i, u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i], u_bbd1[i]:u_bbd2[i]] = \
                    mix_gt[u_rand_index[i], u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i], u_bbd1[i]:u_bbd2[i]]
    if gt != None:
...
class ModelData:
    def __init__(self, outputs, label, labeled_bs, x_range, y_range, z_range, is_first_term=False):
        ...
        self.loss_seg_dice = losses.dice_loss(self.outputs_soft[:labeled_bs, 1, :, :, :], label[:labeled_bs] == 1)
        self.predict = torch.max(self.outputs_soft[:labeled_bs, :, :, :, :], 1,)[1]  
        self.mse_dist = consistency_criterion(self.outputs_soft[:labeled_bs, 1, :, :, :], label[:labeled_bs]) 
    ...
    def add_teacher_loss(self, model_outputs, teacher_outputs, consistency_weight=0):
        ...
        b, v, w, h, d = model_dist.shape 
        model_dist = torch.sum(model_dist) / (b * v * w * h * d)  
    
        self.loss = self.loss + consistency_weight * model_dist

    def get_loss(self, Plabel, consistency_weight=0): # x_range, y_range, z_range, 
    ...
    def get_rot_loss(self, rot_outputs1, rot_outputs2, x_range, y_range, z_range, consistency_weight=0):
        consistency_rot_dist = 0
        for i in range(rot_outputs1.shape[0]):  
            A = self.outputs[i][:, self.x_range[0][i]:self.x_range[1][i], self.y_range[0][i]:self.y_range[1][i], self.z_range[0][i]:self.z_range[1][i]]
            B = rot_outputs1[i][:, x_range[0][i]:x_range[1][i], y_range[0][i]:y_range[1][i], z_range[0][i]:z_range[1][i]]
            C = rot_outputs2[i][:, x_range[0][i]:x_range[1][i], y_range[0][i]:y_range[1][i], z_range[0][i]:z_range[1][i]]
            ...
            consistency_rot_dist += consistency_loss / num_classes  # 除以num_classes
...
def train_model(model_array, teacher_model_array, optimizer_array, data_buffer, model_iter_num, idx, first_term=False,):
    for sampled_batch in data_buffer:
        if first_term:     
            model_output_weak = model_array[idx](sampled_batch[idx]["image_weak"].cuda())
            model_output_strong = model_array[idx]((sampled_batch[idx]["image_strong"]).clamp(min=0.0, max=1.0).cuda())
            teacher_model_output_weak = teacher_model_array[idx](sampled_batch[idx]["image_weak"].cuda())
            teacher_model_output_strong = teacher_model_array[idx]((sampled_batch[idx]["image_strong"]).clamp(min=0.0, max=1.0).cuda())

            data_weak = ModelData(model_output_weak, sampled_batch[idx]["label"].cuda(), labeled_bs,
                            sampled_batch[idx]['x_range'], sampled_batch[idx]['y_range'], sampled_batch[idx]['z_range'], first_term,)
            ...
            for i in range(model_num):  
                if i == idx:
                    model_output_array_weak.append(model_array[i](sampled_batch[i]["image_weak"].cuda()))
                    model_output_array_strong.append(model_array[i]((sampled_batch[i]["image_strong"]).clamp(min=0.0, max=1.0).cuda()))
                    data_arrays_weak.append(ModelData(model_output_array_weak[i], sampled_batch[i]["label"].cuda(), labeled_bs, sampled_batch[i]['x_range'], 
                                        sampled_batch[i]['y_range'], sampled_batch[i]['z_range'], ))
                    data_arrays_strong.append(ModelData(model_output_array_strong[i], sampled_batch[i]["label"].cuda(), labeled_bs, sampled_batch[i]['x_range'],
                                        sampled_batch[i]['y_range'], sampled_batch[i]['z_range'], ))
                    teacher_output_array_weak.append(teacher_model_array[i](sampled_batch[i]["image_weak"].cuda()))
                    teacher_data_arrays_weak.append(ModelData(teacher_output_array_weak[i], sampled_batch[i]["label"].cuda(), labeled_bs, sampled_batch[i]['x_range'],
                                        sampled_batch[i]['y_range'], sampled_batch[i]['z_range'],))
                    teacher_output_array_strong.append(teacher_model_array[i]((sampled_batch[i]["image_strong"]).clamp(min=0.0, max=1.0).cuda()))
                    ...
                    with torch.no_grad():
                        ...
                        model_output_weak = model_array[i](sampled_batch[1-i]["image_weak"].cuda())
                        data_weak = ModelData(model_output_weak, sampled_batch[1-i]["label"].cuda(), labeled_bs, sampled_batch[1-i]['x_range'], 
                                            sampled_batch[1-i]['y_range'], sampled_batch[1-i]['z_range'], )
                        ...
                        teacher_output_weak = teacher_model_array[i](sampled_batch[1-i]["image_weak"].cuda())
            ...
            if Good_student == 1:             
                diff_mask = ((min_seg_model_arrays[0].label[:labeled_bs] == 1) != (min_seg_model_arrays[1].predict == 1))
            elif Good_student == 2:
                diff_mask = ((min_seg_model_arrays[0].label[:labeled_bs] == 1) != (min_seg_model_arrays[2].predict == 1))
            elif Good_student == 3:
                diff_mask1 = ((min_seg_model_arrays[0].label[:labeled_bs] == 1) != (min_seg_model_arrays[1].predict == 1))
                diff_mask2 = ((min_seg_model_arrays[0].label[:labeled_bs] == 1) != (min_seg_model_arrays[2].predict == 1))
                diff_mask = torch.logical_or(diff_mask1, diff_mask2)
                del diff_mask1, diff_mask2

            if  Teacher_Good_student == 1:             
                teacher_diff_mask = ((min_seg_teacher_model_arrays[0].label[:labeled_bs] == 1) != (min_seg_teacher_model_arrays[1].predict == 1))
            elif Teacher_Good_student == 2:
                teacher_diff_mask = ((min_seg_teacher_model_arrays[0].label[:labeled_bs] == 1) != (min_seg_teacher_model_arrays[2].predict == 1))
            elif Teacher_Good_student == 3:
                teacher_diff_mask1 = ((min_seg_teacher_model_arrays[0].label[:labeled_bs] == 1) != (min_seg_teacher_model_arrays[1].predict == 1))
                teacher_diff_mask2 = ((min_seg_teacher_model_arrays[0].label[:labeled_bs] == 1) != (min_seg_teacher_model_arrays[2].predict == 1))
            ...
            # if False:
                data_arrays_weak[idx].get_loss(Plabel, consistency_weight) # data_arrays_weak[1-idx].x_range, data_arrays_weak[1-idx].y_range, data_arrays_weak[1-idx].z_range, 
            else:
                sampled_image_rot1, k, dims_, axis = randomRotFlip(sampled_batch[1-idx]['image_weak'].cuda())
                ...
                sampled_image_rot2, k, dims_, axis = randomRotFlip(sampled_batch[1-idx]['image_weak'].cuda())
                ...
                data_arrays_weak[idx].get_rot_loss(model_output_weak_rot1, model_output_weak_rot2, sampled_batch[1-idx]['x_range'], sampled_batch[1-idx]['y_range'], sampled_batch[1-idx]['z_range'], consistency_weight)
                # data_arrays_weak[idx].get_rot_loss((model_output_weak_rot1 + model_output_weak_rot2) / 2, sampled_batch[1-idx]['x_range'], sampled_batch[1-idx]['y_range'], sampled_batch[1-idx]['z_range'], consistency_weight) # , sampled_batch[1-idx]['x_range'], sampled_batch[1-idx]['y_range'], sampled_batch[1-idx]['z_range']
            ...
            # if False:
                teacher_data_arrays_weak[idx].get_loss(teacher_Plabel, consistency_weight) # teacher_data_arrays_weak[1-idx].x_range, teacher_data_arrays_weak[1-idx].y_range, teacher_data_arrays_weak[1-idx].z_range, 
            else:   
                sampled_image_rot1, k, dims_, axis = randomRotFlip(sampled_batch[1-idx]['image_weak'].cuda())
                ...
                teacher_data_arrays_weak[idx].get_rot_loss(teacher_output_weak_rot1, teacher_output_weak_rot2, sampled_batch[1-idx]['x_range'], sampled_batch[1-idx]['y_range'], sampled_batch[1-idx]['z_range'], consistency_weight)
                # teacher_data_arrays_weak[idx].get_rot_loss((teacher_output_weak_rot1 + teacher_output_weak_rot2) / 2, sampled_batch[1-idx]['x_range'], sampled_batch[1-idx]['y_range'], sampled_batch[1-idx]['z_range'], consistency_weight) # sampled_batch[1-idx]['x_range'], sampled_batch[1-idx]['y_range'], sampled_batch[1-idx]['z_range'], 
            ...
            sampled_batch_mix, plabel_model_output_array_mix = cut_mix((sampled_batch[idx]['image_strong']).clamp(min=0.0, max=1.0)[labeled_bs:].cuda(), model_output_array_weak[idx][labeled_bs:])
            ...
            sampled_batch_mix, plabel_teacher_output_array_mix = cut_mix((sampled_batch[idx]['image_strong']).clamp(min=0.0, max=1.0)[labeled_bs:].cuda(), teacher_output_array_weak[idx][labeled_bs:])
    ...
    if epoch_num >= 120: 
        ...
        teacher_metric = test_calculate_metric(teacher_model_array)
        print('metric: ', metric) 
        print('teacher_metric:', teacher_metric)
        
        if metric[0] > max_score:
            max_score = metric[0]
        ...
        if teacher_metric[0] > teacher_max_score:
            teacher_max_score = teacher_metric[0]
...
if __name__ == "__main__":
    ...
    def create_model(name):
        ...
        if name == "vnet":
            ...
            net  = UNet(in_channels=1, is_batchnorm=True, n_classes=num_classes)
            model = net.cuda()
        return model
    ...
    db_train = LAHeart(base_dir=train_data_path, split="train", train_flod="train.list",  
                       common_transform=transforms.Compose([
                                                            ...
                                                            sp_transform=transforms.Compose([ToTensor_o(),]),
                       )

    labeled_idxs = list(range(8))  # todo set labeled num
    unlabeled_idxs = list(range(8, 80))  # todo set labeled num all_sample_num
    ...
    iter_num = 0
    data_buffer = []
    switch = IntervalSwitch(max_iterations, 1000, 6000)
    teacher_switch = IntervalSwitch(max_iterations, 1000, 6000)
