...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        ...
        image_strong = self.blur(image_strong)
        image = image.astype(numpy.float32)
        image_strong = image_strong.astype(numpy.float32)
        label = label.astype(numpy.uint8)
        return {'image_weak': image, 'image_strong': image_strong, 'label': label}
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        if random.random() < 0.15:
            ...
            image_strong = numpy.pad(image_strong, [(pw, pw), (ph, ph)], mode='constant', constant_values=0)
            ...
            image_strong = image_strong[w1:w1 + self.output_size[0], h1:h1 + self.output_size[1]]
            ...
            image_strong = zoom(image_strong, (self.output_size[0] / x, self.output_size[1] / y), order=0)
            label = zoom(label, (self.output_size[0] / x, self.output_size[1] / y), order=0)
            x, y = image.shape
            cons_image = image.copy()
            cons_strong_image = image_strong.copy()
        ...
        return {'image': image, 'label': label, 'cons_image': cons_image, 'cons_label': cons_label,
                'image_strong': image_strong, 'cons_strong_image': cons_strong_image, 
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        cons_strong_image = sample['cons_strong_image']
        return {'image': torch.from_numpy(image).unsqueeze(0),
                'image_strong': torch.from_numpy(image_strong).unsqueeze(0),
                'label': torch.from_numpy(sample['label']).long(),
                'x_range': sample['normal_range_x'], 'y_range': sample['normal_range_y']}, \
               {'image': torch.from_numpy(cons_image).unsqueeze(0),
                'image_strong': torch.from_numpy(cons_strong_image).unsqueeze(0),
                'label': torch.from_numpy(sample['cons_label']).long(),
